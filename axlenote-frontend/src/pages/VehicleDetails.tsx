    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <Link to="/" className="p-2 rounded-lg bg-neutral-900 border border-white/5 text-neutral-400 hover:text-white transition-colors">
            <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clipRule="evenodd" />
            </svg>
          </Link>
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">{vehicle.name}</h1>
            <p className="text-neutral-400 font-medium">{vehicle.year} {vehicle.make} {vehicle.model}</p>
          </div>
        </div>

        <div className="flex gap-2">
          <button onClick={() => setIsFuelModalOpen(true)} className="flex items-center gap-2 bg-neutral-900 hover:bg-neutral-800 text-white px-4 py-2 rounded-lg font-medium transition-colors border border-white/5 hover:border-emerald-500/20 cursor-pointer">
          ...
          <button onClick={() => setIsReminderModalOpen(true)} className="p-2 bg-neutral-900 hover:bg-neutral-800 text-neutral-300 rounded-lg border border-white/5 hover:border-amber-500/20 transition-colors cursor-pointer" title="Set Reminder">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-neutral-900/50 border border-white/5 rounded-2xl p-5 shadow-lg relative overflow-hidden group">
          ...
          <p className="text-neutral-500 text-xs uppercase font-bold tracking-wider mb-2">Total Lifetime Cost</p>
        ...
        <div className="bg-neutral-900/50 border border-white/5 rounded-2xl p-5">
          <p className="text-neutral-500 text-xs uppercase font-bold tracking-wider mb-2">Monthly Avg. Cost</p>
          <p className="text-3xl font-bold text-white tracking-tight">
            <CountUp end={Math.round(monthlyCost)} prefix="₹" />
          </p>
          <p className="text-xs text-neutral-500 mt-1">Approximate</p>
        </div>
        <div className="bg-neutral-900/50 border border-white/5 rounded-2xl p-5">
          <p className="text-neutral-500 text-xs uppercase font-bold tracking-wider mb-2">Distance Travelled</p>
          <p className="text-3xl font-bold text-white tracking-tight">
        ...
        <div className="bg-neutral-900/50 border border-white/5 rounded-2xl p-5 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:opacity-30 transition-opacity">
          ...
          <p className="text-neutral-500 text-xs uppercase font-bold tracking-wider mb-2">Active Reminders</p>
          <p className="text-3xl font-bold text-white tracking-tight">
        ...
        <nav className="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs">
          {[
            ...
            <button
              ...
              className={clsx(
                activeTab === tab.id ? 'border-emerald-500 text-emerald-500' : 'border-transparent text-neutral-400 hover:text-white hover:border-neutral-700',
      ...
      {/* Content */}
      <div className="bg-neutral-900/40 border border-white/5 rounded-2xl overflow-hidden p-6 min-h-[400px]">
        {/* SERVICES TAB */}
        {activeTab === 'service' && (
          services.length === 0 ? (
            ...
            <div className="space-y-8">
              {services.map((s) => (
                <div key={s.id} className="relative pl-8 border-l border-white/10 last:border-0">
                  <div className="absolute -left-1.5 top-1.5 w-3 h-3 rounded-full bg-neutral-900 border-2 border-emerald-500 ring-4 ring-neutral-900" />
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={clsx("inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border",
                          s.service_type === 'maintenance' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                            s.service_type === 'repair' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="p-1 hover:bg-white/10 rounded text-neutral-500 hover:text-white transition-colors" title="Edit"
                          >
                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                          </button>
                          <button
                            onClick={() => handleDeleteService(s.id)}
                            className="p-1 hover:bg-red-500/10 rounded text-neutral-500 hover:text-red-500 transition-colors" title="Delete"
                          >
                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                          </button>
                        </div>
                      </div>
                      <p className="text-neutral-300 mt-1 whitespace-pre-wrap">{s.notes}</p>
                      {s.document_url && (
                        <a href={s.document_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-2 text-xs font-medium text-violet-400 hover:text-violet-300">
                    ...
                    <div className="text-right sm:shrink-0">
                      <p className="text-white font-bold text-lg">₹{Number(s.cost).toLocaleString('en-IN')}</p>
                      <p className="text-neutral-500 text-sm font-medium">{formatDate(s.date)}</p>
                      <p className="text-neutral-600 text-xs mt-1">{s.odometer.toLocaleString()} km</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          fuelLogs.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/5 text-neutral-500 text-xs uppercase font-bold tracking-wider">
                ...
                <tbody className="divide-y divide-white/5">
                  {fuelLogs.map(f => (
                    <tr key={f.id} className="text-sm group hover:bg-white/5 transition-colors">
                      <td className="py-3 pl-2 text-neutral-300 font-medium">{formatDate(f.date)}</td>
                      <td className="py-3 text-neutral-400 font-mono text-xs">{f.odometer.toLocaleString()} km</td>
                      <td className="py-3 text-neutral-300">{f.liters} L {f.full_tank && <span className="text-emerald-500 ml-1 text-[10px] uppercase font-bold border border-emerald-500/30 px-1 rounded">Full</span>}</td>
                      <td className="py-3 text-right text-neutral-500">₹{f.price_per_liter}</td>
                      <td className="py-3 text-right font-bold text-white">₹{f.total_cost}</td>
                      <td className="py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() => { setEditingFuel(f); setIsFuelModalOpen(true); }}
                            className="text-neutral-500 hover:text-white transition-colors" title="Edit"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                          </button>
                          <button
                            onClick={() => handleDeleteFuel(f.id)}
                            className="text-neutral-500 hover:text-red-500 transition-colors" title="Delete"
                          >
          ...
          reminders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {reminders.map(r => {
                ...
                const borderColor = urgency === 'critical' ? 'border-red-500/50' : urgency === 'warning' ? 'border-amber-500/50' : 'border-white/5'
                const bgGradient = urgency === 'critical' ? 'bg-gradient-to-br from-neutral-900 to-red-900/10' : urgency === 'warning' ? 'bg-gradient-to-br from-neutral-900 to-amber-900/10' : 'bg-neutral-900'
                  ...
                  <div key={r.id} className={`${bgGradient} border ${borderColor} rounded-xl p-4 flex flex-col justify-between hover:border-emerald-500/30 transition-all shadow-lg group`}>
                    <div>
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {r.is_recurring && (
                          <span className="text-[10px] uppercase bg-neutral-800 text-neutral-400 px-1.5 py-0.5 rounded border border-neutral-700">Recurring</span>
                        )}
                      </div>
                      <div className="space-y-1 text-sm text-neutral-400 mb-4">
                        {r.due_date && <p className={urgency === 'critical' && new Date(r.due_date) < new Date() ? "text-red-400 font-bold" : ""}>Due: <span className="text-neutral-200">{formatDate(r.due_date)}</span></p>}
                        {r.due_odometer && <p className={urgency === 'critical' && currentOdo > r.due_odometer ? "text-red-400 font-bold" : ""}>Due Odo: <span className="text-neutral-200">{r.due_odometer.toLocaleString()} km</span></p>}
                        <p className="italic text-neutral-600 text-xs mt-2">{r.notes}</p>
                      </div>
                    </div>
                    <button
                      onClick={async () => {
                      ...
                      className="w-full mt-auto bg-black/20 border border-white/5 hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/20 text-neutral-400 py-2 rounded-lg text-sm transition-all"
                    >
                      Mark Done
                    </button>
                  </div>
          ...
          documents.length === 0 ? (
            ...
            <div>
              <div className="flex justify-end mb-4">
                <button
                  onClick={() => setIsDocumentModalOpen(true)}
                  className="flex items-center gap-2 bg-neutral-900 hover:bg-neutral-800 text-white px-3 py-1.5 rounded-lg text-sm font-medium border border-white/5 transition-colors"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {documents.map(d => (
                  <div key={d.id} className="bg-neutral-900 border border-white/5 p-4 rounded-xl flex items-center justify-between group hover:border-violet-500/30 transition-all">
        ...
        {activeTab === 'analytics' && stats && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* 6 Month Trend */}
              <div className="bg-neutral-900 border border-white/5 p-6 rounded-xl">
                <h3 className="text-lg font-bold text-white mb-6">6-Month Spending Trend</h3>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={trendData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#262626" vertical={false} />
                      <XAxis dataKey="name" stroke="#525252" tick={{ fill: '#737373', fontSize: 12 }} tickLine={false} axisLine={false} />
                      <YAxis stroke="#525252" tick={{ fill: '#737373', fontSize: 12 }} tickLine={false} axisLine={false} tickFormatter={(value) => `₹${value}`} />
                      <Tooltip
                        contentStyle={{ backgroundColor: '#171717', borderColor: '#262626', borderRadius: '0.75rem', color: '#f5f5f5' }}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-neutral-900 p-4 rounded-xl border border-white/5">
                ...
                <p className="text-2xl font-bold text-white">{stats.total_fuel_logs}</p>
              </div>
              <div className="bg-neutral-900 p-4 rounded-xl border border-white/5">
              ...
              <div className="bg-neutral-900 p-4 rounded-xl border border-white/5">
                ...
                <p className="text-2xl font-bold text-white">{stats.total_liters.toFixed(1)} <span className="text-sm font-medium text-neutral-500">L</span></p>
              </div>
              <div className="bg-neutral-900 p-4 rounded-xl border border-white/5">
